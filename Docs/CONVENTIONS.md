...
3. 과거 대화 검색
4. 엔진 로컬 소스 grep — 문서가 답하지 않는 것(정확한 시그니처·기본값·버전별 동작)에만
5. PIE 실측 — 위 넷으로 판정이 안 될 때만, 무엇을 판별하려는지 명시하고

근거는 비용 비대칭이다. 1~3은 도구 호출로 끝나고, 4는 사람의 손과 시간을 쓰며,
5는 거기에 빌드·PIE 시간을 더한다. **grep 요청문을 쓰려는 순간이 「이걸 문서로
찾아봤는가」를 자문할 시점이다.**

확인된 엔진 동작 사실은 `ENGINE_NOTES_GAS.md`에 모은다. 학습 단원 진행 중에는
조회하지 않고 그 파일을 읽는다 — 단원 한가운데서 조사가 터지면 그 단원은 무너진다.

### 이름은 동작과 따로 조사한다

엔진 **동작**은 버전 간 안정적이지만 **이름**(프로퍼티, enum 표시명, 클래스, 매크로,
에디터 메뉴)은 마이너 버전에서도 움직인다. ENGINE_NOTES에 동작이 적혀 있다는 것은
이름까지 확인됐다는 뜻이 아니다.

- Claude의 기억은 조사 출처가 아니다. 5.7 근거를 댈 수 없는 이름은 쓰지 않는다.
- 이름 확인의 1순위는 **로컬 엔진 grep**이다. 공식 문서는 마이너 버전을 못 따라온다.
- 에디터 절차를 안내할 때는 영문 이름과 한글 UI 표기를 함께 적는다.
  대조가 불가능한 안내는 안내가 아니다.
- 확인이 안 된 상태면 절차를 쓰지 말고 grep 요청 한 줄로 대체한다.
